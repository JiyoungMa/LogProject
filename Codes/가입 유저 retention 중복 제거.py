...
while(start + datetime.timedelta(days=7)<datetime.datetime.now()):
    ...
    today_ip = [] #이번 주에 접속한 username이 없는 ip들
    today_user = [] #각 주에 가입한 username들 중 이번 주에 접속한 username들
    for i in range(len(user_id)):
        today_user.append([])
    ...
    for users in json_data:
        if 'username' in users: #username이 있는 경우
            ...
            for i in range(1, now_index+1, 1):
                #user_id의 i번째 인덱스에 속해있는 경우 == i번째 주에 가입함
                #today_user의 i번째 인덱스에 속해있지 않으면 추가함
                # => i번째 주에 가입한 username이 이번 주에 방문했다는 뜻
                if username in user_id[i] and ip not in long_ip and username not in today_user[i]: 
                    today_user[i].append(username)
                    bname = True
                #이미 username이 이번주에 방문한 적이 있는 경우    
                elif username in user_id[i] and ip not in long_ip:
                    bname = True
            ...
            #username이 today_user에 추가가 안 된 경우는, 우리가 가지고 있는 데이터 이 전에 가입한 것이므로
            #today_user[0]에 추가한다.
            if bname == False and username not in today_user[0] and ip not in long_ip:
                today_user[0].append(username)
    ...
    for i in range(0,now_index,1):
        #registered_user : 가입한 유저 중 이번 주에 접속한 모든 user들
        registered_user = today_user[i] #로그인 해서 사용한 경우

        #user_ip의 key 중에 ip가 있다면, 가입유저가 비로그인으로 접속 한 것이라 registered user에 넣어줌
        for ip in today_ip:
            if ip in user_ip[i].keys() and user_ip[i][ip] not in registered_user:
